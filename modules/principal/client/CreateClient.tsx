  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const departamentos = departmentsData.map((dep) => dep.name);
        setDepartamentos(departamentos.sort());
        ...
        for (const dep of departmentsData) {
          ...
          if (!ciudadesResponse.ok) continue;
          const ciudades = await ciudadesResponse.json();
          ciudadesAgrupadas[dep.name] = ciudades.map((city) => city.name);
        }
